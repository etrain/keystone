...
object PCATradeoffs extends Logging{
  ...
  def run(ns: Array[Int], ds: Array[Int], ks: Array[Double], qs: Array[Int], ps: Array[Int], trials: Int) = {
    ...
    for (
      ...
      val (p1, timing) = time(pca.computePCAd(datad, dims))
      logInfo(s"svdPCA,$n,$d,$k,$q,0,$t,$dims,$timing,0.0")

      //Do the approximate PCA

      val (p2, timing2) = time(approximatePCA(datad, dims, q, p))
      val absdiff = abs(p1)-abs(p2)

      logInfo(s"approxPCA,$n,$d,$k,$q,$p,$t,$dims,$timing2,${fro(absdiff)/(n*k)}")
  ...
  case class PCATradeoffConfig(
      ns: Array[Int] = Array(1000,10000,100000),
      ds: Array[Int] = Array(10, 100, 1000, 10000),
      ks: Array[Double] = Array(0.001, 0.01, 0.1, 0.25, 0.5, 0.75, 1.0),
