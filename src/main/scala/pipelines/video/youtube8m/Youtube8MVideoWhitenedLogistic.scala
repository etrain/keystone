...
//
//object Youtube8MVideoWhitenedLogistic extends Serializable with Logging {
...
//    }).cache()
//
//    val X = newData.map(_._2.map(_.toDouble)).cache()
//    val y = newData.map(_._1)
//
//    (X, y)
//  }
//
...
//
//
//    val featurizer = Transformer(toWhiteFormat) andThen
//      whitener andThen
//      Transformer(fromWhiteFormat) andThen
//      new Cacher
//
//    val trainFeatures = featurizer(trainX).get
//
//    val lrModel = new LogisticRegressionEstimator(Youtube8MVideoLoader.NUM_CLASSES, conf.lambda, conf.numIter).fit(trainFeatures, trainLabels)
//
//
//    val testFeatures = featurizer(testData.map(_.features.map(_.toDouble).toDenseVector)))
//    val predictions = lrModel.model.
//
